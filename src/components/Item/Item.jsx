...
class Item extends Component {
  constructor() {
    ...
    this._buttonValidateCreate =this._buttonValidateCreate.bind(this);
    this._buttonCreate = this._buttonCreate.bind(this);
    this._handleDeleteClick = this._handleDeleteClick.bind(this);
    this._handleValidateDeleteClick = this._handleValidateDeleteClick.bind(this);
    this._deleteAttribut = this._deleteAttribut.bind(this);
        ...
        <div className="App-content">
          <div className="Description">
            <div className="DescriptionModality">
              ...
              {!this.state.isCreatable ? this._buttonCreate(): this._buttonValidateCreate()}
              {this.state.isDeletable ?  this._buttonValidateDelete() : this._buttonDelete()}
              <div className="Attributes">
              {attributes}
              {this.state.isCreatable ? this._getForm() : ""}
              </div>
    ...
    return Object.entries(this.state)
      ...
      .map(x => (
        <div className="Attribute" key={x[0]}>
          ...
          {this.state.isDeletable ? this._getFormDeletable(x[0].concat("Button")) : ""}
        </div>
      ));
  }
 ...
 _buttonValidateCreate(){
    return(<button id="buttonCreer" onClick={this._checkIsCreatable}>Valider</button>)
  }

_buttonCreate(){
    return(<button id="buttonCreer" onClick={this._checkIsCreatable}>Creer</button>)
  }
...
_getFormDeletable(key){
     return(
       <button id={key} value="delete" onClick={this._deleteAttribut.bind(this,key)}>X</button>
       );
      }

  _buttonDelete(){
    return(
      <button id= "delete" value="modifier" onClick={this._handleDeleteClick}>Supprimer</button>
    );
  }

  _buttonValidateDelete(){
    return(
      <button value="ok" onClick={this._handleValidateDeleteClick}>Ok</button>
    );
  }
